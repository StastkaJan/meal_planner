...
| POST   | /plans                  | yes  | create plan (`{name, mode?: 'simple'\|'calendar'}`, defaults to `simple`, immutable after creation)                               |
...
| PUT    | /plans/[id]/slots       | yes  | upsert/clear one slot (`{date, mealType, mealId}`); 400 if the meal's `allowedSlots` excludes `mealType` (simple-mode plans only) |
| POST   | /plans/[id]/autocompose | yes  | auto-fill empty week slots (simple-mode plans only — 400 if plan is calendar-mode)                                                |
