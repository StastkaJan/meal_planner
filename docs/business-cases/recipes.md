...
- No image transformations or galleries; uploads are stored as-is, one per
  recipe, with a 5 MB limit.

## Known limitations

- Recipe names, descriptions, ingredient names, and instructions support `en`
  and `cs` translations with field-level fallback to the original recipe.
