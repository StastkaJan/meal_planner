...
Use a dedicated Gmail account. Enable [2-Step Verification and create an app password](https://support.google.com/accounts/answer/185833) named Papu Plan. Put these values in `.env.production` (or `.env` for local development):
...
Use the app password generated by Google, not the account's normal password. Spaces in the displayed app password are stripped automatically. Both app slots receive these variables. The sender is `Papu Plan <your-account@gmail.com>`; a free Gmail account does not require an email subscription or DNS changes for `papuplan.cz`. This implementation sends through `smtp.gmail.com:465` using authenticated TLS, so outbound port 465 must be reachable.
