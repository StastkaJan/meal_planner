...
3. Set `ALERT_WEBHOOK_URL_FILE` in `.env` to that file and start the production
   profile with `docker compose --profile production up -d`.
...
```sh
docker compose --profile production exec backup curl -fsS \
