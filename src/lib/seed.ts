...
import { parseIngredientLine, syncMealIngredients } from './server/meals.js'
...
if (existingMeals.length === 0) {
  ...
  const linesByName = new Map(mealDefs.map((m) => [m.name, m.ingredients]))
  await db.transaction(async (tx) => {
    for (const m of inserted) {
      const lines = linesByName.get(m.name) ?? []
      await syncMealIngredients(tx, m.id, lines.map(parseIngredientLine))
    }
  })
