...
async function fetchRecipeHtml(startUrl: string): Promise<string> {
  ...
  for (let hop = 0; hop < 4; hop++) {
    if (!isPublicHttpUrl(target)) error(400, 'URL must be a public http(s) address');
    let res: Response;
    try {
    ...
    if (Number(res.headers.get('content-length')) > MAX_BODY_BYTES) error(413, 'Recipe page is too large');
