<script lang="ts">
  ...
  async function importRecipe() {
    ...
    try {
      ...
      if (!res.ok) {
        importError = (await res.json().catch(() => ({}))).message ?? 'Import failed';
        return;
      }
...
<div class="page">
  <div class="top-bar">
    ...
    <div class="top-actions">
      <button class="btn ghost" onclick={() => { importing = !importing; importError = ''; }}>Import from URL</button>
  ...
  {#if importing}
    <div class="import-bar">
      <input type="url" placeholder="https://…recipe page URL" bind:value={importUrl}
        onkeydown={(e) => { if (e.key === 'Enter') importRecipe(); }} />
      <button class="btn sm" onclick={importRecipe} disabled={importBusy}>{importBusy ? 'Importing…' : 'Import'}</button>
      <button class="btn sm ghost" onclick={() => { importing = false; importError = ''; }}>Cancel</button>
...
<style lang="scss">
  ...
  .import-bar {
    ...
    .import-error { color: $color-danger; font-size: 0.8rem; }
  }
