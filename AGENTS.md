...
<!-- NOTE: `/auth/forgot-password` and `/auth/reset-password` use JSON POST endpoints. `password_resets` keeps one 30-minute hashed token per account, tied to the current password; redemption atomically consumes it and revokes sessions. Email uses Resend (`RESEND_API_KEY`, `EMAIL_FROM`, trusted `ORIGIN`); background delivery failures appear as `auth/request_password_reset` service errors. -->
