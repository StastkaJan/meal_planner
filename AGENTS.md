...
- This project does **not** use `invalidate`/`invalidateAll`. Interactive/in-place mutations (slot pick, settings toggle, autocompose, delete) call `fetch()` against the REST endpoint, then update local state: derive a writable copy of load data with `$derived` (e.g. `let plan = $derived(data.plan)`) and reassign it after the `fetch` — see `handleSlotChange`/`handleSettingsChange` in `src/routes/+page.svelte`. A mutation that changes which rows exist (delete plan) just `goto()`s to re-run `load`.
- Use a `+page.server.ts` form action + `use:enhance` only where there's a real form with inputs (`createPlan` on the home page, create/delete on `meals/+page.svelte`) — not for button- or picker-triggered mutations.
- Note: navigating via `use:enhance`'s redirect doesn't remount the component, so any local `$state` tied to "is this form open" (e.g. `creating`) must be reset explicitly in a custom `use:enhance` callback — see `createPlan` in `src/routes/+page.svelte`.
