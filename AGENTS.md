...
- Mutations go through `+page.server.ts` form actions (`use:enhance`), not `fetch()` + `invalidate()`/`invalidateAll()`. The default `use:enhance` behavior already refreshes `data` (and follows redirects) after a successful submit — don't hand-patch local copies of server state, and don't call `invalidate`/`invalidateAll` yourself.
- For mutations triggered from non-form UI (a picker callback, a debounced toggle), use a hidden `<form use:enhance>` submitted programmatically via `formEl.requestSubmit()` rather than reaching for `fetch()` — see `src/routes/+page.svelte`'s `slotChange`/`settingsChange` forms.
