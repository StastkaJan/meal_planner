...
- This project does **not** use `invalidate`/`invalidateAll`. Component-driven mutations call `fetch()` against the REST endpoint (`src/routes/{plans,meals}/**/+server.ts`), then update local state directly: for an in-place edit, derive a writable copy of load data with `$derived` (e.g. `let plan = $derived(data.plan)`) and reassign it after the `fetch` (see `handleSlotChange`/`handleSettingsChange` in `src/routes/+page.svelte`); for a create/delete that changes which rows exist, `goto()` the new/`/` URL to re-run `load` (see `createPlan`/`deletePlan`).
- Form actions (`+page.server.ts` `actions` + `use:enhance`) are used only for genuine `<form>` submissions — auth, profile, and the meal create/edit/delete on `meals/+page.svelte` / `meals/[id]`. Don't add form actions for button- or picker-triggered mutations.
- Note: `goto()` (and a `use:enhance` redirect) to the same route doesn't remount the component, so local `$state` for "is this form open" (e.g. `creating`) must be reset explicitly in the handler — see `createPlan` in `src/routes/+page.svelte`.
