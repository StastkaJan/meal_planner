...
1. Registration requires terms acceptance and privacy acknowledgement; their shared timestamp and version are stored on `users`. Register/login → `createSession()` creates a `sessions` row and sets the httpOnly cookie. Login uses a constant-time dummy hash when the user is absent.
